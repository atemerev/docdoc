...
exports.isBlankBeforeOcr = isBlankBeforeOcr;
 ...
 */
function isBlankBeforeOcr(imagePath, existingText = "") {
    ...
    try {
        ...
        while (edgeHead < edgeTail) {
            ...
            for (let ny = Math.max(0, y - 1); ny <= Math.min(h - 1, y + 1); ny++)
                for (let nx = Math.max(0, x - 1); nx <= Math.min(w - 1, x + 1); nx++)
                    if (!(nx > w * 0.08 && nx < w * 0.92 && ny > h * 0.08 && ny < h * 0.92))
                        seed(nx, ny);
        }
        ...
        for (let start = 0; start < gray.length; start++) {
            ...
            while (head < tail) {
                ...
                if (x > w * 0.08 && x < w * 0.92 && y > h * 0.08 && y < h * 0.92)
                    interior++;
                for (let ny = Math.max(0, y - 1); ny <= Math.min(h - 1, y + 1); ny++)
                    for (let nx = Math.max(0, x - 1); nx <= Math.min(w - 1, x + 1); nx++) {
                        const next = ny * w + nx;
            ...
            if (edgeShadow || registration || tail < 6 || Math.max(cw, ch) < 3)
                continue;
            if (darkest >= 175) {
                if (ch < h * 0.03 && foldRows.some((y) => top >= y - h * 0.02 && bottom <= y + h * 0.02))
                    continue;
        ...
        if (smallMarks.some((a) => smallMarks.filter((b) => Math.abs(a.y - b.y) <= Math.max(a.height, b.height) && Math.abs(a.x - b.x) < w * 0.1).length >= 3))
            return false;
        return true;
    }
    catch {
