...
exports.paperLoaded = paperLoaded;
...
async function probeEscl() {
    for (let port = 60000; port < 60010; port++) {
        ...
        try {
            const res = await fetch(`${base}/eSCL/ScannerStatus`, { signal: AbortSignal.timeout(3000) });
            if ((await res.text()).includes("AdfState"))
                return base;
        }
...
async function discover() {
    setupSaneConfig();
    try {
        const { stdout } = await (0, exec_1.run)("scanimage", ["-L"], { env: { ...process.env, SANE_CONFIG_DIR: SANE_DIR }, timeout: 30000 });
...
async function online() {
    if (Date.now() - onlineCache.at < 30000)
    ...
    try {
        if (!esclBase)
            esclBase = await probeEscl();
        if (esclBase) {
            const res = await fetch(`${esclBase}/eSCL/ScannerStatus`, { signal: AbortSignal.timeout(3000) });
            onlineCache.value = (await res.text()).includes("AdfState");
            if (!onlineCache.value)
                esclBase = null;
        }
    }
    catch {
...
async function paperLoaded() {
    ...
    if (!esclBase)
        return false;
    const res = await fetch(`${esclBase}/eSCL/ScannerStatus`, { signal: AbortSignal.timeout(5000) });
    return (await res.text()).includes("ScannerAdfLoaded");
}
const countPages = (dir) => fs.readdirSync(dir).filter((f) => /^page-\d+\./.test(f)).length;
function runScanimage(dir, ext, startAt) {
    return new Promise((resolve) => {
        const proc = (0, child_process_1.spawn)("scanimage", [
            "-d", devname,
            "--source", SCAN.source,
            "--mode", SCAN.mode,
        ...
        current.proc = proc;
        (0, exec_1.trackChild)(proc);
        let out = "";
        proc.stdout.on("data", (d) => { out += d; });
        proc.stderr.on("data", (d) => { out += d; });
        proc.on("close", () => {
            (0, exec_1.untrackChild)(proc);
            current.proc = null;
            for (const line of out.split("\n"))
                if (line.trim())
                    console.log(`scanimage: ${line}`);
            resolve();
        });
    });
}
...
async function scanNow() {
    ...
    void runBatch(dir, flag).catch((e) => console.log(`scan: ${e.message}`));
    return true;
}
