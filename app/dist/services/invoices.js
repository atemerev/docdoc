...
exports.recordInvoice = recordInvoice;
exports.chainIds = chainIds;
exports.markPaid = markPaid;
exports.markDoNotPay = markDoNotPay;
...
async function recordInvoice(con, cfg, documentId, senderId, ext, qr) {
    ...
    if (!due && ext.doc_date && ext.doc_type === "invoice") {
        ...
        due = (0, qrbill_1.addDays)(ext.doc_date, term);
    ...
    if (level > 0) {
        ...
        if (parent)
            ...
            notes.push("reminder: no matching open invoice found");
    }
...
async function findParent(con, cfg, senderId, ext, qr, excludeId, notes) {
    ...
    const norms = ext.refs
        ...
        .map((r) => (0, textsim_1.normRef)(r.value))
        .filter((n) => n.length >= 4);
    if (norms.length) {
...
function markPaid(con, invoiceId, { note = null, accountId = null, paidDate = null } = {}) {
    const at = paidDate || (0, db_1.nowIso)();
    const ids = chainIds(con, invoiceId);
    const upd = con.prepare(`UPDATE invoices SET status='paid', paid_at=?, paid_note=?,
         paid_account_id=? WHERE id=?`);
    for (const id of ids)
        upd.run(at, note, accountId, id);
    return ids;
}
/**
...
function markDoNotPay(con, invoiceId, note = null) {
    const ids = chainIds(con, invoiceId);
    const upd = con.prepare(`UPDATE invoices SET status='void', paid_at=?, paid_note=?,
         paid_account_id=NULL WHERE id=?`);
    for (const id of ids)
        upd.run((0, db_1.nowIso)(), note, id);
    return ids;
}
function reopen(con, invoiceId) {
    const ids = chainIds(con, invoiceId);
    const upd = con.prepare(`UPDATE invoices SET status='open', paid_at=NULL, paid_note=NULL,
         paid_account_id=NULL WHERE id=?`);
    for (const id of ids)
        upd.run(id);
    return ids;
