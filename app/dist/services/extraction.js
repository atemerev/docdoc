...
exports.runExtraction = exports.resetExtractor = exports.setExtractor = exports.EXTRACTION_SCHEMA = exports.slugify = void 0;
exports.stripFences = stripFences;
exports.normalize = normalize;
exports.extractClaudeCli = extractClaudeCli;
exports.extractLocalVllm = extractLocalVllm;
exports.matchReminder = matchReminder;
exports.extractHeuristic = extractHeuristic;
exports.extract = extract;
const fs = __importStar(require("fs"));
const textsim_1 = require("../domain/textsim");
Object.defineProperty(exports, "slugify", { enumerable: true, get: function () { return textsim_1.slugify; } });
...
function normalize(d, qr = null) {
    ...
    const out = {
        ...
        refs: (Array.isArray(d.refs) ? d.refs : [])
            ...
            .map((r) => ({
            kind: types_1.REF_KINDS.includes(r.kind) ? r.kind : "other",
...
function qrContext(qr) {
    ...
    if (qr.creditor)
        lines.push(`  creditor: ${qr.creditor.name}`);
    if (qr.amount != null)
        lines.push(`  amount: ${qr.amount} ${qr.currency}`);
    if (qr.reference)
        lines.push(`  reference: ${qr.reference}`);
    if (qr.is_notification)
        lines.push("  NOTE: amount 0.00 = notification, not payable");
    return lines.join("\n") + "\n";
}
function buildPrompt(p) {
    ...
    return PROMPT
        ...
        .replace("{doc_types}", types_1.DOC_TYPES.join("|"))
        .replace("{ref_kinds}", types_1.REF_KINDS.join("|"))
...
async function extractClaudeCli(imagePaths, ocrText, { qr = null, knownSenders = [], maxPages = 4, timeout = 300000, model = "sonnet" } = {}) {
    ...
    const { stdout } = await (0, exec_1.run)("claude", ["-p", prompt, "--allowedTools", "Read",
        "--output-format", "json", "--model", model], { env: envWithoutAnthropicKey(), timeout });
...
exports.EXTRACTION_SCHEMA = {
    type: "object",
    properties: {
        doc_type: { type: "string", enum: [...types_1.DOC_TYPES] },
        ...
        refs: { type: "array", maxItems: 20, items: {
                type: "object",
                properties: { kind: { type: "string", enum: [...types_1.REF_KINDS] },
                    value: { type: "string" } },
...
async function vllmModel(baseUrl) {
    const res = await fetch(baseUrl.replace(/\/$/, "") + "/models", { signal: AbortSignal.timeout(5000) });
    const body = await res.json();
    return body.data[0].id;
}
async function extractLocalVllm(imagePaths, ocrText, { qr = null, knownSenders = [], maxPages = 4, timeout = 300000, baseUrl = "http://localhost:8000/v1" } = {}) {
    const images = imagePaths.slice(0, maxPages);
    const prompt = buildPrompt({
    ...
    const res = await fetch(base + "/chat/completions", {
        ...
        body: JSON.stringify({
            ...
            response_format: { type: "json_schema", json_schema: {
                    name: "extraction", schema: exports.EXTRACTION_SCHEMA
                } },
        }),
...
async function matchReminder(cfg, ext, candidates, timeout = 120000) {
    ...
    try {
        ...
        if (cfg.ai_provider === "local-vllm") {
            ...
            const { stdout } = await (0, exec_1.run)("claude", ["-p", prompt, "--output-format", "json",
                "--model", cfg.ai_model || "sonnet"], { env: envWithoutAnthropicKey(), timeout });
...
function extractHeuristic(ocrText, qr = null) {
    ...
    const dt = d.doc_type;
    d.title = { invoice: "Rechnung", reminder: "Mahnung" }[dt] ?? dt[0].toUpperCase() + dt.slice(1);
    d.tags = dt !== "other" ? [dt] : [];
    const refs = [];
...
async function extract(cfg, imagePaths, ocrText, { qr = null, knownSenders = [] } = {}) {
    ...
    if (cfg.ai_provider === "local-vllm") {
        try {
            return { ext: await extractLocalVllm(images, ocrText, { ...opts, baseUrl: cfg.ai_base_url }), provider: "local-vllm" };
        }
        catch (e) {
    ...
    if (["claude-cli", "local-vllm"].includes(cfg.ai_provider)) {
        try {
            return { ext: await extractClaudeCli(images, ocrText, { ...opts, model: cfg.ai_model || "sonnet" }), provider: "claude-cli" };
        }
        catch (e) {
...
let activeExtract = extract;
const setExtractor = (fn) => { activeExtract = fn; };
exports.setExtractor = setExtractor;
const resetExtractor = () => { activeExtract = extract; };
exports.resetExtractor = resetExtractor;
const runExtraction = (...args) => activeExtract(...args);
