...
const datePattern = /\b(?:\d{4}-\d{2}-\d{2}|\d{1,2}[./]\d{1,2}[./]\d{4}|\d{1,2}(?:er|\.)?\s+[\p{L}]+\s+\d{4})\b/gu;
function parseDate(text) {
    if (/^\d{4}-/.test(text))
        return (0, exports.validDate)(text);
    const parts = fold(text).match(/^(\d{1,2})(?:er|\.)?[\s./]+([a-z]+|\d{1,2})[\s./]+(\d{4})$/);
    if (!parts)
        return null;
    const month = /^\d+$/.test(parts[2])
        ? Number(parts[2])
        : months.findIndex((m) => m.split(" ").includes(parts[2])) + 1;
    return (0, exports.validDate)(`${parts[3]}-${String(month).padStart(2, "0")}-${parts[1].padStart(2, "0")}`);
}
...
function documentDates(text, pursuit = false) {
        ...
        for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
            ...
            for (const match of line.matchAll(datePattern)) {
                ...
                const prefix = fold(before
                    ...
                    .filter(Boolean)
                    .pop() ||
                    lines.slice(Math.max(0, lineIndex - 2), lineIndex).join(" ")).trim();
                const evidence = line
                    .trim()
                    .replace(/\s{3,}/g, " · ")
                    .slice(0, 240);
                let kind = "mentioned", label = "Mentioned date", score = 0;
                if (/\b(ne\(e\)|ne|nee|naissance|geburt|geboren|birth|born|nato|nata|enfants?|children)\b/.test(prefix) ||
                    /ne\(e\)/.test(prefix)) {
                    kind = "birth";
                    label = "Birth date";
                }
                else if (/(?:ouverture|introduction|initiation|opened|eroffnung|einleitung|apertura).{0,45}(?:poursuite|procedure|dossier|case|verfahren)?|(?:requisition de poursuite|betreibungsbegehren)\s*(?:du|vom|le)?\s*$/.test(prefix)) {
                    kind = "case_opened";
                    label = "Case initiated";
                }
                else if (/(?:execution|pfandung|saisie|pignoramento)\s*:?\s*$/.test(prefix)) {
                    kind = "execution";
                ...
                else if (/(?:echeance|payable|due date|fallig|zahlbar|scadenza).{0,25}$/.test(prefix)) {
                    kind = "due";
                    label = "Due date";
                }
                else if (/(?:constat|feststellung|observation).{0,20}$/.test(prefix)) {
                    kind = "observation";
                    label = "Observation";
                }
                else if (/(?:date (?:du document|d.emission)|document date|issued|ausgestellt|datum|data di emissione)\s*:?\s*$/.test(prefix) ||
    ...
    const chosen = dates.length === 1
        ? candidates.find((c) => c.score === best && c.date === dates[0])
        : null;
    const opened = [
        ...new Set(events.filter((e) => e.kind === "case_opened").map((e) => e.date)),
    ];
    return {
        ...
        ref_dates: events.filter((e, i) => events.findIndex((o) => o.date === e.date && o.kind === e.kind && o.page === e.page) === i),
    };
}
