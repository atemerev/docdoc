...
exports.correspondenceMetadata = correspondenceMetadata;
const textsim_1 = require("./textsim");
const handlerLabel = /(?:^|[ \t]{2,})(?:dossier[ \t]+trait[ée][ \t]+par|(?:dossier|affaire)[ \t]+suivi[e]?[ \t]+par|votre[ \t]+interlocut(?:eur|rice)|personne[ \t]+en[ \t]+charge|gestionnaire|sachbearbeiter(?:in)?|bearbeitet[ \t]+von|handled[ \t]+by|case[ \t]+(?:handler|officer)|contact[ \t]+person)[ \t]*:?[ \t]*/i;
...
function identifier(value) {
    ...
    const match = /^(?:[A-Z]{1,6}[ \t]+)?(?=[A-Z0-9./-]*\d)[A-Z0-9][A-Z0-9./-]*(?:[ \t]+(?=[A-Z0-9./-]*\d)[A-Z0-9][A-Z0-9./-]*)*(?:[ \t]+\/[ \t]+[A-Z]{2,6}\b)?/i.exec(field);
    const result = match?.[0].replace(/[.\s]+$/, "");
    return result && (0, textsim_1.normRef)(result).length >= 4 ? result : null;
}
function correspondenceMetadata(text, pursuit = false) {
    const refs = [];
    const handlers = [];
        ...
        for (const [i, line] of lines.entries()) {
            ...
            if (handler) {
                ...
                if (name &&
                    ...
                    for (const contact of lines.slice(nameLine + 1, nameLine + 5)) {
                        if (referenceLabels.some((rx) => rx.test(contact)) ||
                            handlerLabel.test(contact))
                            break;
                        if (!contact.trim())
                            break;
                        const mail = /[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/i.exec(contact)?.[0];
                        const tel = /^[ \t]*(?:(?:t(?:[ée]l(?:[ée]phone)?)?|phone|direct|telephone)\.?[ \t]*:?[ \t]*)?(\+?\d[\d ()/.-]{6,}\d)[ \t]*$/i.exec(contact)?.[1];
                        if (!mail && !tel)
                            break;
                        email ||= mail || null;
                        phone ||= tel || null;
            ...
            for (const [labelIndex, regex] of referenceLabels.entries()) {
                for (const label of line.matchAll(new RegExp(regex.source, "gi"))) {
                    ...
                    if (!rest.trim()) {
                        const column = label.index + (label[0].match(/^[ \t]*/)?.[0].length || 0);
                        const next = lines
                            .slice(i + 1, i + 4)
                    ...
                    if (value)
                        refs.push({
                            ...
                            value,
                            page: pageIndex + 1,
                            evidence: `${line.trim()}${line.slice(label.index + label[0].length).trim() ? "" : `\n${rest.trim()}`}`.slice(0, 240),
                        });
                }
            }
        }
    ...
    for (const ref of refs) {
        ...
        if (suffix &&
            refs.some((other) => other.kind === ref.kind &&
                (0, textsim_1.normRef)(other.value) === (0, textsim_1.normRef)(suffix[1]))) {
            ref.value = suffix[1];
            codes.add(suffix[2]);
        }
    }
    ...
    return {
        case_handler,
        refs: refs.filter((ref, i) => refs.findIndex((other) => other.kind === ref.kind &&
