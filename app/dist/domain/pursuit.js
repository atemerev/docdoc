...
exports.PURSUIT_WORDS = void 0;
exports.pursuitMetadata = pursuitMetadata;
const textsim_1 = require("./textsim");
exports.PURSUIT_WORDS = /acte de d[ée]faut de biens|commandement de payer|proc[eè]s.verbal de saisie|avis de saisie|(?:poursuite|betreibung|esecuzione)\s*(?:n[°ºo.]|nr\.?|nummer|\n\s*\d)|Verlustschein|Zahlungsbefehl|Pf[äa]ndungs(?:urkunde|ank[üu]ndigung)|attestato di carenza di beni|precetto esecutivo/i;
const identifier = "([A-Z0-9][A-Z0-9./-]*(?:[ \\t]+[0-9]+)*(?:[ \\t]+[A-Z](?![a-zA-Z]))?)";
const patterns = [
    [
        "pursuit_no",
        new RegExp("(?:poursuite|betreibung|esecuzione)[ \\t]*(?:n[°ºo.]|nr\\.?|nummer)[ \\t]*:?[ \\t]*" +
            identifier, "gi"),
    ],
    [
        "debt_certificate_no",
        new RegExp("(?:ADB|Verlustschein|attestato)[ \\t]*(?:n[°ºo.]|nr\\.?|nummer)[ \\t]*:?[ \\t]*" +
            identifier, "gi"),
    ],
];
function pursuitMetadata(text) {
    const refs = [];
    const parties = [];
    const add = (kind, value, page, evidence) => {
        value = value.replace(/[.\s]+$/, "").trim();
        if (!/\d/.test(value) ||
            (0, textsim_1.normRef)(value).length < 4 ||
            refs.some((r) => r.kind === kind && (0, textsim_1.normRef)(r.value) === (0, textsim_1.normRef)(value)))
            return;
    ...
    for (const [pageIndex, page] of text.split("\f").entries()) {
        ...
        for (const [index, line] of rawLines.entries()) {
            const label = /\b(?:Poursuite|Betreibung|Esecuzione)\s*:?\s*$/i.exec(line);
            if (!label)
                continue;
            // Follow the label's column, not the next postal address in OCR order.
            for (const next of rawLines.slice(index + 1, index + 5)) {
                const match = new RegExp("^[ \\t]*" + identifier).exec(next.slice(Math.max(0, label.index - 8)));
                if (match && (0, textsim_1.normRef)(match[1]).length >= 7) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            const role = /^(?:repr[ée]sentant du cr[ée]ancier|gl[aä]ubigervertreter)\s*:?$/i.test(label)
                ? "representative"
            ...
            const claim = /^\[\d+\]\s*([A-Z0-9-]{4,})\s*,\s*(?:Facture|Rechnung|Invoice)\s+du\s+\d{2}[./]\d{2}[./]\d{4}\s*,\s*([A-Z0-9-]{4,})/i.exec(lines[i]);
            if (claim) {
    ...
    const subtype = /acte de d[ée]faut de biens|Verlustschein|attestato di carenza/i.test(text)
