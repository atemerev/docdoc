// docdoc renderer -- vanilla-JS SPA over the preload bridge.

import * as pdfjsLib from "./pdfjs/pdf.mjs";
pdfjsLib.GlobalWorkerOptions.workerSrc = "app://ui/pdfjs/pdf.worker.mjs";

const api = (method, params) => window.docdoc.call(method, params);
...
window.docdoc.onEvent((msg) => {
  if (msg.event === "changed") {
    clearTimeout(refreshTimer);
    refreshTimer = setTimeout(() => {
      refreshCounts();
      render();
      if (state.detailId) openDetail(state.detailId, { keepPdf: true });
    }, 400);
  }
});
...
async function refreshCounts() {
  try {
    ...
    $("#count-inbox").textContent = s.inbox || "";
    $("#count-docs").textContent = s.documents || "";
    $("#count-unpaid").textContent = s.unpaid_count || "";
    $("#sb-stats").innerHTML =
...
function docRow(d, { snippet = false } = {}) {
  ...
  return `
    <div class="doc-row ${d.id === state.detailId ? "selected" : ""}" data-id="${d.id}">
      ...
      <div class="doc-side">
        ${d.amount ? `<div class="doc-amount">${fmtAmount(d.amount, d.currency)}</div>` : ""}
...
async function renderDocuments(c) {
  let docs, heading = "All documents";
  if (state.searchQ) {
  ...
  const [senders, years] = await Promise.all([api("list_senders"), api("years")]);
  ...
  bindDocRows(c);
  $("#f-type").onchange = (e) => { f.doc_type = e.target.value; render(); };
  $("#f-sender").onchange = (e) => { f.sender_id = e.target.value; render(); };
...
async function renderInvoices(c) {
  const [stats, invs, act] = await Promise.all([
    api("stats"),
    api("list_invoices", { status: state.invoiceFilter === "all" ? null : state.invoiceFilter }),
    api("status"),
  ]);
  // documents still in the pipeline: a just-scanned invoice has no
  // invoices row yet -- say so instead of silently not listing it
  const inflight = (act.scanning?.length || 0) + (act.waiting?.length || 0)
                 + (act.queued?.length || 0) + (act.processing?.length || 0);
  c.innerHTML = `
    <h2>Invoices</h2>
    <div class="stats">
    ...
    <table><thead><tr>
      ...
      ${invs.map((i) => {
        ...
        const st = i.status === "paid" ? "paid"
                 ...
                 : i.overdue ? "overdue"
                 : i.max_reminder_level > 0 ? "reminded"
                 : i.status;
  ...
  $$("[data-f]", c).forEach((b) =>
    b.addEventListener("click", () => { state.invoiceFilter = b.dataset.f; render(); }));
  $$("tr.click", c).forEach((tr) =>
    tr.addEventListener("click", () => openDetail(parseInt(tr.dataset.doc, 10))));
  $$("[data-pay]", c).forEach((b) =>
    b.addEventListener("click", (e) => {
      e.stopPropagation();
      const id = parseInt(b.dataset.pay, 10);
      showPayDialog(id, invs.find((i) => i.id === id),
...
// IBAN: ISO 13616 -- shown grouped by 4, stored compact, mod-97 integrity
// check mirrors qrbill.iban_valid on the Python side
const ibanCompact = (s) => String(s || "").replace(/\s+/g, "").toUpperCase();
const ibanGroup = (s) => ibanCompact(s).replace(/(.{4})/g, "$1 ").trim();
function ibanValid(iban) {
  ...
  const digits = (s.slice(4) + s.slice(0, 4))
    .replace(/[A-Z]/g, (ch) => ch.charCodeAt(0) - 55);
  let rem = 0;
...
async function renderAccounts(c, adding = false) {
  ...
  $$("tr[data-aid]", c).forEach((tr) => {
    const ib = $(".a-iban", tr), msg = $(".iban-msg", tr);
    ...
    ib.oninput = () => {
      ...
      const at = ib.value.slice(0, ib.selectionStart).replace(/\s+/g, "").length;
  ...
  const add = $("#a-add");
  if (add) add.onclick = () => renderAccounts(c, true);
}
...
async function renderActivity(c) {
  ...
  $$("[data-doc]", c).forEach((a) =>
    a.addEventListener("click", () => openDetail(parseInt(a.dataset.doc, 10))));
}
...
async function renderSettings(c) {
  ...
  c.innerHTML = `
    ...
    <div class="settings-form">
      ${field("ai_provider", "AI provider", ["claude-cli", "none"])}
  ...
  $("#save-settings").onclick = async () => {
    ...
    kv.default_payment_term_days = parseInt(kv.default_payment_term_days, 10) || 30;
...
async function openDetail(id, { keepPdf = false } = {}) {
  ...
  $("#d-close").onclick = () => { panel.classList.add("hidden"); state.detailId = null; render(); };
  $("#d-open").onclick = () => window.docdoc.openExternal(id);
  $("#d-folder").onclick = () => window.docdoc.openFolder(id);
  ...
  if (rev) rev.onclick = async () => {
    ...
    refreshCounts(); openDetail(id, { keepPdf: true }); render();
  };
  if (inv) {
    const pay = $("#i-pay"), reopen = $("#i-reopen"), qr = $("#i-qr");
    if (pay) pay.onclick = () =>
      showPayDialog(inv.id, { ...inv, sender_name: d.sender_name },
                    () => { refreshCounts(); render(); openDetail(id, { keepPdf: true }); });
    if (reopen) reopen.onclick = async () => {
      await api("invoice_reopen", { id: inv.id }); refreshCounts(); openDetail(id, { keepPdf: true });
    };
    if (qr) qr.onclick = () => showQr(inv.id, inv);
  }
  $$("[data-goto]", panel).forEach((a) =>
    a.addEventListener("click", () => openDetail(parseInt(a.dataset.goto, 10))));

  if (!keepPdf || pdfStale || !$("#pdf-pages canvas")) renderPdf(id, d.pages || 1);
}

async function renderPdf(id, nPages) {
...
async function showPayDialog(invoiceId, inv = null, onDone = null) {
  const accounts = await api("list_bank_accounts");
  const m = $("#modal");
  ...
  $("#pay-dnp").onchange = (e) => {
    $("#pay-fields").classList.toggle("hidden", e.target.checked);
    $("#pay-note-label").classList.toggle("hidden", !e.target.checked);
    $("#pay-ok").textContent = e.target.checked ? "Do not pay" : "Mark paid";
    if (e.target.checked) $("#pay-note").focus();
  };
...
// ------------------------------------------------------------------ QR modal
async function showQr(invoiceId, inv = null) {
  try {
...
let scanError = null;               // { msg, at }
let abortArmedAt = 0;               // first Abort click arms, second fires
let abortRequestedAt = 0;           // abort_scan sent, pipeline still busy
const shownPct = {};                // batch -> displayed %, creeps between polls

const pill = (html, cls = "") => `<span class="status-pill ${cls}">${html}</span>`;

function processingPill(p) {
  if (p.pct == null)                // no progress file (CLI run) -> indeterminate bar
...
async function pollStatus() {
  ...
  for (const b of s.waiting || [])
    pills.push(s.watcher_alive
      ...
      : pill(`⚠ ${esc(b)} will not process — docdocd is not running`, "warn"));
  ...
  if (ab) ab.onclick = async () => {
    if (Date.now() - abortArmedAt < 4000) {
      ...
      try {
        ...
        abortRequestedAt = 0;
        scanError = { msg: e.message.replace(
          /^Error invoking remote method '[^']*': (Error: )?/, ""), at: Date.now() };
      }
    } else {
...
const DOT_TITLES = {
  "#sb-scanner": {
    bad: "scan-buttond is not running — the scan button won't work",
    off: "scanner not connected — power on / plug in the ADS-4300N",
    ready: "scan-buttond — ready to scan",
    active: "scan-buttond — scanning",
  },
  "#sb-watcher": {
    bad: "docdocd is not running — new scans won't be processed",
    ready: "docdocd — idle, watching for new scans",
    active: "docdocd — processing a batch",
  },
};

function setDot(sel, state) {
  const d = $(`${sel} .sb-dot`);
  d.classList.toggle("bad", state === "bad");
  d.classList.toggle("off", state === "off");
  d.classList.toggle("ready", state === "ready");
  d.classList.toggle("ok", state === "active");
  $(sel).title = DOT_TITLES[sel][state];
}
