...
import type { BankAccountRow, Config, DocType, DocumentRow, EventRow,
              InvoiceRow, SenderRow } from "../domain/types";
...
export class Api {
  ...
  list_documents({ doc_type = null, sender_id = null, year = null,
                   inbox = false, limit = 200, offset = 0 } = {} as Params) {
  ...
  get_document({ id }: { id: number }) {
    ...
    const inv = this.con.prepare(
      "SELECT * FROM invoices WHERE document_id=?").get(id) as
    ...
    d.duplicates = (this.con.prepare(
      "SELECT id FROM documents WHERE duplicate_of=?").all(id) as
  ...
  timeline({ id }: { id: number }): TimelineEntry[] {
    ...
    const add = (date: string | null, label: string, kind: string,
                 docId: number | null = null): void => {
    ...
    try {
      const ai = JSON.parse(doc.ai_json || "{}") as
        { ref_dates?: Array<{ date: string; label?: string }> };
      ...
      for (const cid of invoices.chainIds(this.con, inv.id)) {
        const m = this.con.prepare(
          `SELECT i.*, dd.doc_date AS ddate, dd.title AS dtitle,
           ...
           WHERE i.id=?`).get(cid) as InvoiceRow &
          { ddate: string | null; dtitle: string | null; did: number };
  ...
  update_document({ id, ...fields }: { id: number } & Params) {
    ...
    for (const [k, v] of Object.entries(fields)) {
      if (allowed.has(k)) {
        ...
        const sid = name
          ? db.upsertSender(this.con, slugify(name), name) : null;
        sets.push("sender_id=?", "sender_name=?");
        args.push(sid, fold(name || ""));
      }
    }
    if (sets.length)
      this.con.prepare(
  ...
  list_senders(): SenderRow[] {
    return this.con.prepare(
      `SELECT s.*, COUNT(d.id) AS doc_count,
       ...
       GROUP BY s.id ORDER BY doc_count DESC`).all() as SenderRow[];
  }
  ...
  invoice_paid({ id, note = null, account_id = null, paid_date = null }:
      { id: number; note?: string | null; account_id?: number | null;
  ...
  save_bank_account({ id = null, holder = null, bank = null, iban = null }:
      { id?: number | null; holder?: string | null; bank?: string | null;
        iban?: string | null } = {}) {
    holder = (holder || "").trim();
    bank = (bank || "").trim();
    iban = (iban || "").trim();
  ...
  async render_qr({ invoice_id }: { invoice_id: number }) {
    const row = this.con.prepare(
      ...
      Pick<InvoiceRow, "qr_payload" | "amount" | "amount_due"> | undefined;
    if (!row?.qr_payload)
  ...
  // -- misc ---------------------------------------------------------------
  async _pgrep(pattern: string): Promise<number[]> {
    try {
  ...
  async abort_scan() {
    ...
    if (fs.existsSync(sd)) {
      ...
      try { fs.unlinkSync(path.join(sd, ".scanning")); } catch { /* absent */ }
    }
  ...
  async status() {
    ...
    interface ScanInfo { batch: string; pages: number }
    const scanning: ScanInfo[] = [];
    ...
    interface Processing {
      batch: string; label: string | null;
    ...
    if (fs.existsSync(tmpdir)) {
      for (const f of fs.readdirSync(tmpdir)) {
        ...
        try {
          ...
          const prog = JSON.parse(fs.readFileSync(p, "utf-8")) as
            { label?: string; pct?: number; ceil?: number };
    ...
    ).all() as Array<{ id: number; batch: string | null }>)
      .map((r) => r.batch || `#${r.id}`);
    ...
    for (const r of this.con.prepare(
        `SELECT batch, kind FROM events
         ...
         ORDER BY id`).all(cutoff) as
        Array<Pick<EventRow, "batch" | "kind">>) {
    ...
    const waiting: string[] = [], queued: string[] = [];
    if (fs.existsSync(sd)) {
      for (const name of fs.readdirSync(sd).sort()) {
        const p = path.join(sd, name);
        let isDir = false;
        try { isDir = fs.statSync(p).isDirectory(); } catch { /* raced */ }
  ...
  stats() {
    ...
    const types = this.con.prepare(
      `SELECT doc_type, COUNT(*) n FROM documents WHERE status!='trash'
       GROUP BY doc_type ORDER BY n DESC`).all() as
      Array<{ doc_type: DocType | null; n: number }>;
  ...
  set_settings(kv: Params = {}): Config {
    ...
    config.save(cfg as unknown as Config);
    this.cfg = config.load();
    return this.cfg;
  }
  ...
  years(): string[] {
    ...
    ).all() as Array<{ y: string }>).map((r) => r.y);
  }
}
