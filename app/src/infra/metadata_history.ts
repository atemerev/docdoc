...
export function initializeMetadataHistory(con: Db): void {
  ...
  con.transaction(() => {
    ...
    con.exec(
      "UPDATE documents SET scanned_at=created_at,scan_date_source='legacy_recorded_at' WHERE scanned_at IS NULL",
    );
    const rows = con
      .prepare(
        "SELECT id FROM documents WHERE id NOT IN (SELECT document_id FROM document_metadata_versions)",
      )
...
export function recordMetadata(con: Db, id: number, source: string): void {
  con.transaction(() => {
    const doc = con
      .prepare(
        `SELECT title,doc_type,sender_id,sender_name,recipient,doc_date,case_opened_date,scanned_at,scan_date_source,amount,currency,ai_json FROM documents WHERE id=?`,
      )
    ...
    const previous = con
      .prepare(
        "SELECT * FROM document_metadata_versions WHERE document_id=? AND recorded_to IS NULL",
      )
      .get(id) as
      | { id: number; recorded_from: string; snapshot: string }
      | undefined;
    if (previous?.snapshot === snapshot) return;
    const now = new Date(
      Math.max(
        Date.now(),
        previous ? Date.parse(previous.recorded_from) + 1 : 0,
      ),
    ).toISOString();
    if (previous)
      con
        .prepare(
          "UPDATE document_metadata_versions SET recorded_to=? WHERE id=?",
        )
        .run(now, previous.id);
    con
      .prepare(
        "INSERT INTO document_metadata_versions(document_id,valid_from,recorded_from,source,snapshot) VALUES (?,?,?,?,?)",
      )
      .run(id, doc.doc_date ?? null, now, source, snapshot);
  })();
}
export function metadataAsOf(
  con: Db,
  id: number,
  knownAt: string,
  effectiveOn?: string,
  ...
  if (effectiveOn && !validDate(effectiveOn))
    throw new Error("A valid effective date is required.");
  const known = new Date(knownAt).toISOString();
  const row = con
    .prepare(
      `SELECT * FROM document_metadata_versions WHERE document_id=? AND recorded_from<=? AND (recorded_to IS NULL OR recorded_to>?) ${effectiveOn ? "AND valid_from<=? AND (valid_to IS NULL OR valid_to>?)" : ""} ORDER BY recorded_from DESC LIMIT 1`,
    )
    .get(
      id,
      known,
      known,
      ...(effectiveOn ? [effectiveOn, effectiveOn] : []),
