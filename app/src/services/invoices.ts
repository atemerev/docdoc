...
function findParent(
  ...
  if (qr?.reference) {
    const row = q(
      `SELECT i.* FROM invoices i WHERE i.qr_reference = ?
                   AND ${ROOT}`,
      [qr.reference],
    );
    if (row) return row;
  ...
  if (ext.invoice_ref) {
    const row = q(
      `SELECT i.* FROM invoices i WHERE i.invoice_ref = ?
                   AND ${ROOT}`,
      [ext.invoice_ref],
    );
    if (row) return row;
  ...
  const norms = ext.refs
    .filter((r) =>
      ["invoice_no", "order_no", "case_no", "other"].includes(r.kind),
    )
...
function linkReminder(
  ...
  notes.push(
    `linked to invoice #${parent.id}` + (fee ? `, fee ${fee.toFixed(2)}` : ""),
  );
}
...
export function chainIds(con: Db, invoiceId: number): number[] {
  ...
  return (
    con
      ...
      .all(root, root) as Array<{ id: number }>
  ).map((r) => r.id);
}
