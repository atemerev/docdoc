...
export function refDuplicate(
  ...
  if (ext.doc_type === "pursuit") return { id: null, reason: null };
...
export function findDuplicates(
  ...
  let row = con
    .prepare(
      "SELECT id FROM documents WHERE file_sha256=? AND status!='trash' AND id!=?",
    )
    .get(sha, excludeId) as { id: number } | undefined;
  if (row) return { id: row.id, reason: "exact-file" };
  row = con
    .prepare(
      "SELECT id FROM documents WHERE text_hash=? AND status!='trash' AND id!=?",
    )
    .get(thash, excludeId) as { id: number } | undefined;
