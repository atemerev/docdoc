...
import { pageMarker, checkOrder } from "../domain/pageorder";
import { fold, slugify } from "../domain/textsim";
import type { Config, DocumentRow, Extraction, QrBill, RefKind }
  from "../domain/types";
import * as db from "../infra/db";
import { BatchAborted, aborted, checkAbort } from "../infra/exec";
import { isBlank } from "../infra/imaging";
import * as ocr from "../infra/ocr";
import { findQrbill } from "../infra/qrcodec";
import { findDuplicates, fileSha256, refDuplicate } from "./dedup";
import { textHash } from "../domain/textsim";
...
const progressPath = (cfg: Config, batch: string): string =>
...
export function progress(
  cfg: Config, batch: string, stage: string, label: string,
  pct: number, ceil: number,
): void {
  checkAbort();
...
export function clearProgress(cfg: Config, batch: string): void {
  try { fs.unlinkSync(progressPath(cfg, batch)); } catch { /* absent */ }
}

const IMAGE_EXTS = new Set([".jpg", ".jpeg", ".png", ".tif", ".tiff", ".pnm"]);

export function batchImages(batchDir: string): string[] {
  let entries: string[];
 ...
 */
function batchPdf(batchDir: string): string | null {
  let entries: string[];
...
export function archiveName(
  ext: Partial<Extraction>, docId: number, title: string | null,
): string {
  const date = ext.doc_date || new Date().toISOString().slice(0, 10);
  const year = date.slice(0, 4);
  const titleSlug = slugify(title || ext.doc_type || "document").slice(0, 48);
...
export async function ingestBatch(
  cfg: Config, con: db.Db, batchDir: string,
): Promise<number | null> {
  ...
  try {
    ...
    let thumbRel: string | null = `${String(docId).padStart(5, "0")}.jpg`;
    try {
...
export async function processDocument(
  cfg: Config, con: db.Db, docId: number, imagesDir: string | null = null,
): Promise<number> {
...
async function understand(
  cfg: Config, con: db.Db, doc: DocumentRow, images: string[],
  rawPdf: string, workdir: string,
): Promise<number> {
  ...
  const qr = images.length ? await findQrbill(images) : null;
  if (qr)
  ...
  const imgs: Array<string | null> =
    images.length === n ? images : new Array(n).fill(null);

  // 3. blank pages
  const blanks = new Set<number>();
  if ((cfg.blank_page_drop ?? true) && n > 1) {
    for (let i = 0; i < n; i++)
      if (isBlank(pageTexts[i], imgs[i],
          { minChars: Number(cfg.min_chars_nonblank) || 12 }))
        blanks.add(i);
  ...
  const known = (con.prepare("SELECT key FROM senders").all() as
    Array<{ key: string }>).map((r) => r.key);
  const scanContent = kept.map((i) => pageTexts[i]).join("\n\f\n");
  progress(cfg, batch, "ai", "AI reading the document", 58, 90);
  const { ext, provider } = await runExtraction(cfg,
    kept.map((i) => imgs[i]).filter((p): p is string => p !== null),
    scanContent, { qr, knownSenders: known });
  ...
  if (groups) {
    ...
    db.event(con, "info", `stack split into ${groups.length} documents`,
    ...
    for (let gi = 0; gi < groups.length; gi++) {
      ...
      const subQr = subImages.length ? await findQrbill(subImages) : null;
      progress(cfg, batch, "ai",
               `AI reading document ${gi + 1}/${groups.length}`,
               58 + Math.round((gi / groups.length) * 32), 90);
      const sub = await runExtraction(cfg, subImages,
  ...
  const { order: markerOrder, flags: oflags } =
    checkOrder(kept.map((i) => pageTexts[i]));
...
async function fileDocument(
  cfg: Config, con: db.Db, doc: DocumentRow, ext: Extraction,
  qr: QrBill | null, finalPdf: string, kept: number[], keptTexts: string[],
  blanks: Set<number>, flags: string[], pageTexts: string[],
): Promise<void> {
  const docId = doc.id, batch = doc.batch;
  ...
  const sha = fileSha256(finalPdf);
  const thash = textHash(content);
  let { id: dupOf, reason: dupReason } =
    findDuplicates(con, sha, thash, content);
  ...
  if (!dupOf) {
    const { id: rid, reason: rreason } = refDuplicate(con, ext, content);
  ...
  con.prepare(
    `UPDATE documents SET doc_date=?, title=?, doc_type=?, sender_id=?,
     ...
     WHERE id=?`
        ...
        fold(ext.sender_name || ""), ext.recipient_name,
        ext.language, ext.summary_en, JSON.stringify(ext.tags),
        fold(tagsText), rel, thumbRel, kept.length, fold(content),
  ...
  const refs: Array<[RefKind, string]> =
    ext.refs.map((r) => [r.kind, r.value] as [RefKind, string]);
  ...
  kept.forEach((scanIdx, i) => {
    const marker = pageMarker(keptTexts[i]);
  ...
  if ((["invoice", "reminder"].includes(ext.doc_type) || qr) && !dupOf) {
    const { notes } = await recordInvoice(con, cfg, docId, senderId, ext, qr);
...
export function finishBatch(
  cfg: Config, batchDir: string, success: boolean,
  keep: boolean | null = null,
): string | null {
  const batch = path.basename(batchDir.replace(/\/+$/, ""));
