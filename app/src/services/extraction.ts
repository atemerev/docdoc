...
import { documentDates, validDate } from "../domain/document_dates";
import { PURSUIT_WORDS, pursuitMetadata } from "../domain/pursuit";
import { correspondenceMetadata } from "../domain/correspondence";
import {
  matchSender,
  senderFromHeader,
  senderName,
...
export function normalize(
  ...
  const out: Extraction = {
    ...
    case_opened_date: validDate(d.case_opened_date),
    date_evidence: str(d.date_evidence) || null,
    pursuit: d.pursuit ? (d.pursuit as PursuitDetails) : null,
    case_handler: d.case_handler ? (d.case_handler as CaseHandler) : null,
    ...
    refs: (Array.isArray(d.refs) ? d.refs : [])
      ...
      .map(
        (r): ExtractedRef => ({
          kind: REF_KINDS.includes(r.kind as RefKind)
            ? (r.kind as RefKind)
            : "other",
      ...
      .map((r) => {
        ...
        return date
          ? {
              ...
              kind: rr!.kind as import("../domain/types").ExtractedDate["kind"],
  ...
  if (qr) {
    ...
    if (
      !["invoice", "reminder", "pursuit"].includes(out.doc_type) &&
      !qr.is_notification
    )
...
function printedSiteName(text: string): string | undefined {
      ...
      const label = line
        ...
        .find(
          (part) =>
            ...
            part.length <= 60 &&
            host.some(
              (name) => senderName(part).replace(/ /g, "") === senderName(name),
            ),
        );
...
export function extractHeuristic(
  ...
  if (PURSUIT_WORDS.test(text)) d.doc_type = "pursuit";
  ...
  Object.assign(d, documentDates(text, d.doc_type === "pursuit"));
  if (d.doc_type !== "pursuit" && validDate(qr?.swico?.invoice_date)) {
  ...
  if (qr?.creditor && d.doc_type !== "pursuit") {
    ...
    const known = senderFromHeader(senders, text);
    ...
    for (let line of text
      ...
      if (
        ...
        line.length <= 80 &&
        !/^\d/.test(line) &&
        !/^(direct print|printed|print.out|page |seite |to:|from:|date:|an:|datum:|dear |sehr geehrte|madame|monsieur)/i.test(
          line,
        ) &&
  ...
  const knownSender = d.sender_name
    ? matchSender(senders, String(d.sender_name), {
        uid: qr?.swico?.uid,
        iban: qr?.iban,
      })
    : undefined;
  if (knownSender) d.sender_name = knownSender.name;
  ...
  const heading = text
    ...
    .find(
      (line) =>
        ...
        line.length <= 100 &&
        !/https?:\/\/|\s\|\s/.test(line) &&
        senderName(line) !== senderName(String(d.sender_name || "")) &&
  ...
  d.title =
    text.trim() || qr
      ...
      : null;
  d.tags = dt !== "other" ? [dt] : [];
  const correspondence = correspondenceMetadata(text, d.doc_type === "pursuit");
  d.case_handler = correspondence.case_handler;
  const refs: ExtractedRef[] = [...correspondence.refs];
  ...
  if (d.doc_type === "pursuit") {
    const pursuit = pursuitMetadata(text);
    d.pursuit = pursuit.pursuit;
    d.refs = [...pursuit.refs, ...refs].filter(
      (ref, i, all) =>
        all.findIndex(
          (other) => other.kind === ref.kind && other.value === ref.value,
        ) === i,
    );
