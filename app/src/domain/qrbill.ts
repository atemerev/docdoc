...
import type { QrAddress, QrBill, SwicoS1 } from "./types";

export const QR_IID_MIN = 30000;
export const QR_IID_MAX = 31999;
export const MOD10_TABLE = [0, 9, 4, 6, 8, 2, 7, 1, 3, 5] as const;
...
const alnumDigits = (s: string): string =>
...
export function qrrValid(ref: string | null | undefined): boolean {
  const s = ref ?? "";
  if (!/^\d{27}$/.test(s) || /^0+$/.test(s)) return false;
  let carry = 0;
...
export function scorValid(ref: string | null | undefined): boolean {
...
function swicoSplit(s: string): string[] {
  const parts: string[] = [];
  let cur = "", i = 0;
  while (i < s.length) {
    const c = s[i];
 ...
 */
export function parseSwico(strd: string | null | undefined): SwicoS1 | null {
  ...
  const out: SwicoS1 = { scheme: "S1", tags, raw: strd };
  ...
  if ("40" in tags) {
    const discounts: Array<[number, number]> = [];
...
function address(lines: string[]): QrAddress | null {
  const [adrtp, name, l1, l2, pcode, town, country] =
    [...lines, "", "", "", "", "", "", ""].slice(0, 7);
  if (!name) return null;
  const a: QrAddress = { name, country: country || null, type: adrtp || null };
  if (adrtp === "K") {
 ...
 */
export function parseSpc(payload: string | null | undefined): QrBill | null {
