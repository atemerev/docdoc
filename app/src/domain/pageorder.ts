...
export interface OrderCheck {
  order: number[] | null;   // 1-based: final position -> scanned page
  flags: string[];
}
...
export function pageMarker(text: string | null | undefined): Marker | null {
...
export function checkOrder(pageTexts: Array<string | null>): OrderCheck {
  const n = pageTexts.length;
  if (n <= 1) return { order: null, flags: [] };
  const markers = pageTexts.map(pageMarker);
  const known = markers
    .map((m, i) => [i, m] as const)
    .filter((e): e is readonly [number, Marker] => e[1] !== null);
