"""SQLite schema and helpers. One writer (pipeline/server), WAL mode.
...
# OR IGNORE make concurrent first connections (server, watchd) idempotent
PRESET_ACCOUNTS = [
...
def connect(path=None):
    ...
    seed_accounts = not con.execute(
        """SELECT 1 FROM sqlite_master WHERE type='table'
           AND name='bank_accounts'""").fetchone()
    _migrate(con)
...
def _migrate(con):
    ...
    if cols and "pending" not in cols:
        con.execute("ALTER TABLE documents ADD COLUMN pending TEXT")
        con.commit()
    cols = {r[1] for r in con.execute("PRAGMA table_info(invoices)")}
    if cols and "paid_account_id" not in cols:
        con.execute("""ALTER TABLE invoices ADD COLUMN paid_account_id INTEGER
...
def event(con, kind, message, batch=None, document_id=None, at=None):
    import datetime
    con.execute(
        "INSERT INTO events(at, kind, batch, document_id, message) VALUES (?,?,?,?,?)",
        (at or datetime.datetime.now().isoformat(timespec="seconds"),
         kind, batch, document_id, message))
    con.commit()


def upsert_sender(con, key, name, uid=None, iban=None, address=None):
    """Create or enrich a sender; never overwrite good data with null."""
    row = con.execute("SELECT * FROM senders WHERE key=?", (key,)).fetchone()
    if row is None:
        cur = con.execute(
            "INSERT INTO senders(key, name, uid, iban, address) VALUES (?,?,?,?,?)",
            (key, name, uid, iban, json.dumps(address) if address else None))
        return cur.lastrowid
    con.execute(
        """UPDATE senders SET name=COALESCE(?, name), uid=COALESCE(?, uid),
           iban=COALESCE(?, iban), address=COALESCE(?, address) WHERE id=?""",
        (name, uid, iban, json.dumps(address) if address else None, row["id"]))
    return row["id"]
...
def add_refs(con, document_id, refs):
    """refs: iterable of (kind, value). Skips short/duplicate values."""
    seen = set()
    for kind, value in refs:
        n = norm_ref(value)
        if len(n) < 4 or n in seen:      # too short to be a meaningful ID
            continue
        seen.add(n)
        con.execute(
            "INSERT INTO doc_refs(document_id, kind, value, norm) VALUES (?,?,?,?)",
            (document_id, kind, str(value).strip(), n))


def related_documents(con, document_id):
...
def search(con, query, limit=100):
    """FTS search-as-you-type: each token quoted, last token prefixed."""
    tokens = [t.replace('"', "") for t in fold(query).split() if t.strip('*"')]
    if not tokens:
        return []
